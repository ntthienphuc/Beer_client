# app.py  –  Beer Manager with GPIO-triggered camera  
# Mỗi lon bia che cảm biến (BCM 17) ⇒ chụp 1 ảnh, nhận diện, cộng món
# ====================================================================
...
GPIO_PULL_UP     = False  # True nếu cảm biến xuất LOW khi bị che
GPIO_BOUNCE_TIME = 0.3    # giây: 1 lon = 1 ảnh

# ─── ĐƯỜNG DẪN & MODEL TFLite ───────────────────────────────────────
...
gpio_btn    = None
cap_lock    = threading.Lock()            # tránh trùng camera

# ─── HÀM CHỤP & DỰ ĐOÁN “ÂM THẦM” ─────────────────────────────────
def _capture_and_predict(add_fn):
    if not cap_lock.acquire(blocking=False):
        return                            # camera đang bận
    try:
...
def build_order():
    ...
    if not order_state["active"]:
        ...
        if gpio_btn: gpio_btn.close(); gpio_btn = None
        return
    ...
    inner  = ttk.Frame(canvas); vsb = ttk.Scrollbar(grp, orient=VERTICAL, command=canvas.yview)
    ...
    def add_item(n):
        ...
        append_bill(bill,n,qtys[n],menu[n]); update_total()
    def sub_item(n):
        if qtys[n]==0:return
        qtys[n]-=1; tree.set(n,"qty",qtys[n])
        append_bill(bill,n,qtys[n],menu[n]); update_total()
    ...
    ttk.Button(ctrl, text="📷 Upload Ảnh Bia", width=20, bootstyle="info",
               command=upload).pack(pady=(0,10))

    ttk.Button(ctrl, text="✅ Kết thúc", width=20, bootstyle="primary",
               command=finish_session).pack()

    # Khởi tạo GPIO (chỉ một lần)
    if GPIO_AVAILABLE and gpio_btn is None:
        gpio_btn = Button(GPIO_PIN, pull_up=GPIO_PULL_UP, bounce_time=GPIO_BOUNCE_TIME)
        def on_blocked():
            if cap_lock.locked():      # camera đang bận
                return
...
def build_admin():
    ...
    ttk.Label(f, text="⚙️ TRANG ADMIN", font=('Segoe UI Semibold',16),
...
def build_history():
    ...
    filt = ttk.LabelFrame(f, text="Bộ lọc", padding=10)
    filt.pack(fill=X, padx=10, pady=8)
    date_var, start_var, end_var = tk.StringVar(), tk.StringVar(), tk.StringVar()
    def _row(t,v,c,w=12):
        ttk.Label(filt, text=t).grid(row=0, column=c, sticky=W, padx=4)
        ttk.Entry(filt, textvariable=v, width=w).grid(row=0, column=c+1, pady=2)
    ...
    tree = ttk.Treeview(lst, columns=("Bàn","BĐ","KT","Ngày","Tổng"), show="headings")
    ...
    def load_rows():
        ...
        try:
            ...
            messagebox.showerror("Sai định dạng","Ngày phải dd-mm-YYYY", parent=root); return
        try:
            us, ue = _parse(start_var.get()), _parse(end_var.get())
        except ValueError:
            messagebox.showerror("Sai định dạng","Giờ phải HH:MM", parent=root); return
